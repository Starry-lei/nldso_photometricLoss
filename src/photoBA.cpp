...
bool next_step( ){
	...
	std::string dataFolder="/home/lei/Documents/Research/nldso_photometricLoss/dataAnalysis/seq_15/";
	...
	for(; (frame = dataset->getFrame(fid, lvl)) && !gStop; ++fid ){
		if (fid==T_init.size()-3) {
			...
			auto output_fn = dataFolder+ "refined_poses_es_tum_abs_pose_pure_pba_seq15_new"+ std::to_string(lvl)+ ".txt";
			writePosesTumRGBDFormat(output_fn, result.poses, dataset->getTimestamp());
			return false;
		}
		printf("Frame %05d\n", fid);
