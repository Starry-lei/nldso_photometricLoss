...
namespace DSONL
{
        ...
        //These are static pointer to a Renderer used in the glut callback functions
	...
	static diffuse_Mask_Renderer *render_diffuse_mask = NULL;
        ...
        //TODO: release these static pointers???
	...
	static void glutKeyboard(unsigned char key, int x, int y)
	{
        ...
        static void glutKeyboard_specular(unsigned char key, int x, int y)
        {
          ...
          if (key >= '1' && key <= '9')
          {
            renderer_specular->selectedOutput = int(key) - int('1');
          }
        }
...
//        void preFilter::modify_shader_file(string roughness_str, string envmapLvl_str) const
//        {
//          string shader_txt = "../include/shaders/fragment_shader.txt";
//          string shader_txt_out = "../include/shaders/fragment_shader_modified.txt";
//          char buff[256];
//          std::cmatch m;
//          fstream setRoughness(shader_txt);
...
//          roughness == (float)0.2 ? envmapLvl = 0.0 : envmapLvl = roughness * 5.0;
//          oss_envl << std::setprecision(2) << envmapLvl;
//          envmapLvl_str = oss_envl.str() + ".0";
//          modify_shader_file(roughness_str, envmapLvl_str);
...
//                                                                                  double min_depth_val, max_depth_val;
//                                                                                  cv::minMaxLoc(mask, &min_depth_val, &max_depth_val);

//                                                                              for (int u = 0; u < input.rows; u++) // colId, cols: 0 to 1024
//                                                                              {
//                                                                                for (int v = 0; v < input.cols; v++) // rowId,  rows: 0 to 512
//                                                                                {
//                                                                                  if (mask.at<float>(u, v) ==0.0f){
...
//                                                                              double min_depth_val, max_depth_val;
//                                                                              cv::minMaxLoc(mask, &min_depth_val, &max_depth_val);
//                                                                              cout<<"show mask range: min and max"<<min_depth_val<<","<< max_depth_val<<endl;
////
///
/// range of mask show mask range: min and max0.00392157,1

 //        gsn       show image_ref_path_PFM range:1.53794e-05,1
          ...
          for (int u = 0; u < input.rows; u++) // colId, cols: 0 to 1024
          {
            for (int v = 0; v < input.cols; v++) // rowId,  rows: 0 to 512
            {
              ...
              if (mask.at<float>(u, v) ==0.0f)
              {
                ...
                //          cout<<"show 1.0 / mask.at<float>(u, v):"<<1.0 / mask.at<float>(u, v)<<endl;
        ...
        void EnvMapLookup::mergeImagePyramid() {
          ...
          std::vector<cv::Mat> img_pyramid_mask_test;
            ...
            for (int i = 0; i < 6; ++i)
            {
              ...
              //imshow("image_pyramid[i]_before",image_pyramid[i]);
...
//              cv::merge(final_channels, image_pyramid[i]);
              Mat mergedMat;
              merge(final_channels, mergedMat);
              img_pyramid_mask_test.push_back(mergedMat);
              image_pyramid[i]=img_pyramid_mask_test[i].clone();


              //// show the min max val
              //              double min_depth_val, max_depth_val;
              //              cv::minMaxLoc(image_pyramid[i], &min_depth_val, &max_depth_val);
              //              Mat imageshow=image_pyramid[i];
              //              cout << "\n "<<"imageshow depth():"<<imageshow.depth()<<" And image_pyramid[i] range:\n"<<min_depth_val<<","<<max_depth_val<<endl;
                            //              imageshow=imageshow * (1.0 / (max_depth_val - min_depth_val)) +(-min_depth_val * (1.0 / (max_depth_val - min_depth_val)));
              //              imshow("image_pyramid[i]_after",imageshow);
              //              waitKey(0);
            }
...
//                     << SampleAAAAAA.b << "," << SampleAAAAAA.g << "," << SampleAAAAAA.r << "," << SampleAAAAAA.a << endl;
	...
	public:
		brdfIntegrationMap();
		~brdfIntegrationMap();
		void makebrdfIntegrationMap(); //float &NoV, float &roughness
...
//                imshow("brdfIntegration_Map",brdfIntegration_Map);

        }

	void brdfIntegrationMap::makebrdfIntegrationMap() //float &NoV, float &roughness
	{
...
//		gli::vec4 SampleBrdf = Sampler_brdf.texture_lod(gli::fsampler2D::normalized_type(0.5f,0.25f), 0.0f); // transform the texture coordinate
//		cout << "\n============SampleBrdf val(RGBA):\n"
//			 << SampleBrdf.b << "," << SampleBrdf.g << "," << SampleBrdf.r << "," << SampleBrdf.a << endl;
//
//
//                gli::vec4 SampleBrdf_test = brdfSampler->texture_lod(gli::fsampler2D::normalized_type(0.5f,0.25f), 0.0f); // transform the texture coordinate
//                cout << "\n============SampleBrdf_test val(RGBA):\n"
//                     << SampleBrdf_test.b << "," << SampleBrdf_test.g << "," << SampleBrdf_test.r << "," << SampleBrdf_test.a << endl;
//
...
//		gli::vec4 SampleDiffuseMask = Sampler_diffuse_mask.texture_lod(gli::fsampler2D::normalized_type(0.5, 0.25), 0.0f);
	}
...
//                waitKey(0);
	...
	gli::vec4 diffuseMap::getDiffuse(float u, float v)
	{
        ...
        void diffuseMap::mergeDiffuse_Map() {
          ...
          cout<<"====================check diffuse_Final_Map size============="<<diffuse_Final_Map.rows<<
