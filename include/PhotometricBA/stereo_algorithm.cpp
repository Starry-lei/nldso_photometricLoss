...
struct StereoAlgorithm::Impl
{
  ...
  Impl(const utils::ConfigFile& cf)
...
//      _sgbm = std::make_unique<cv::StereoSGBM>(
...
//    }
//    else
 if(icompare("BlockMatching", alg) || icompare("BM", alg))
    {
...
RSGM::RSGM(Config conf) : _config(conf), _impl(make_unique<Impl>()) {}
RSGM::~RSGM() {}
...
RSGM::RSGM(Config config) :
    _config(config), _impl(std::make_unique<Impl>()) {}
