	...
	void plotImPyr(FrameHessian* fh, int i, std::string ImgName )
{
	...
	void showImage(const FrameHessian * fh,const float* map_out){
		...
		cv::Mat img(h, w, CV_32FC3, cv::Scalar(0,0,0));



		for (int i = 0; i < h; ++i) {
			cv::Vec3f * p3= img.ptr<cv::Vec3f>(i);

			for (int j = 0; j < w; ++j) {
				float c = fh->dI[i*w+j][0]; // 像素值
				// col: j, row: i
				if(map_out[i*w+j] == 1){
					for(int idx=-3;idx<=3;idx++)
					{
						img.at<cv::Vec3f>(j+3,i+idx) = cv::Vec3f(0,255,0);
						img.at<cv::Vec3f>(j-3,i+idx) =  cv::Vec3f(0,255,0);
						img.at<cv::Vec3f>(j+2,i+idx) = cv::Vec3f(0,255,0);
						img.at<cv::Vec3f>(j-2,i+idx) =  cv::Vec3f(0,255,0);

						img.at<cv::Vec3f>(j+idx,i-3) =  cv::Vec3f(0,255,0);
						img.at<cv::Vec3f>(j+idx,i+3) =  cv::Vec3f(0,255,0);
						img.at<cv::Vec3f>(j+idx,i-2) =  cv::Vec3f(0,255,0);
						img.at<cv::Vec3f>(j+idx,i+2) =  cv::Vec3f(0,255,0);
					}
				}
