...
namespace dsm
{
	...
	// Incomplete Beta function
	 float beta(float x, float a, float b, Tail tail = lower);

	// Regularized Incomplete Beta function
	// Ix = beta(x,a,b)/beta(a,b)
	 float betainc(float x, float a, float b, Tail tail = lower);

	// Inverse Beta function
	// Ix^-1 = beta(x,a,b)/beta(a,b) = p
	 float betaincinv(float p, float a, float b, Tail tail = lower);
}