...
//
//        Eigen::Matrix<float,3,1> reflection(out_vec.val[0], out_vec.val[1],out_vec.val[2]);
////        Eigen::Quaternionf q_1(0.6570,   -0.7441 ,   0.0907,   -0.0801); //  cam1  wxyz
//        Eigen::Matrix3f trueR_1w;
//        trueR_1w <<-0.9707,   -0.0298,    0.2384,
//                                   -0.2403,    0.1203,   -0.9632,
//                                   0 ,  -0.9923,   -0.1240;
////        reflection =q_1.toRotationMatrix()*reflection;
////        reflection=reflection.normalized();
//
//
//        reflection= trueR_1w*reflection;
//        reflection=reflection.normalized();

//        return Vec3f(reflection.x(), reflection.y(), reflection.z());
        return  out_vec;
    }
...
//      return prefiltered_Color_check;// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    void updateDelta(
      ...
      for (int u = 0; u< depth_map.rows; u++) // colId, cols: 0 to 480
      {
        for (int v = 0; v < depth_map.cols; v++) // rowId,  rows: 0 to 640
        {
          ...
          beta_prime=-CurrentT.rotationMatrix().transpose().cast<float>()*CurrentT.translation().cast<float>()-p_c1;
          ...
          Vec3f radiance_beta =  ibl_Radiance->solveForRadiance(View_beta,N_,image_roughnes,image_metallic,reflectance,baseColor,Camera1_c2w);
          Vec3f radiance_beta_prime = ibl_Radiance->solveForRadiance(View_beta_prime,N_,image_roughnes,image_metallic,reflectance,baseColor,Camera1_c2w);

          radianceMap_left.at<Vec3f>(u,v)= radiance_beta;


//          if (u==290 && v== 130){
//            // std::cout<<"show View_beta in my left camera:"<<View_beta<<std::endl;
//            std::cout<<"show Normal in my left camera:"<<N_<<std::endl;
//            std::cout<<"show Refelection:"<< ibl_Radiance->specularIBLCheck( Vec3f (normal(0),normal(1),normal(2)), 0.5, N_, View_beta,Camera1_c2w)<<std::endl;
//          }

          // GSN: show depth pixel coordinate(290,130)And values:[-0.0888623, 0.576571, 0.8122]
...
//      imshow("Initial deltaMap",deltaMap);
        //colorDeltaMap
...
//                imshow("coloredDeltaMap", coloredDeltaMap);
//                imshow("UpperLower deltaMap",deltaMap);
